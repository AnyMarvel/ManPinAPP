package com.mp.android.apps.monke.monkeybook;
...
public class ProxyManager {
    ...
    public static void saveProxyState(boolean state) {
        proxyState = state;
        SharedPreferences.Editor editor = MyApplication.getInstance().getSharedPreferences("CONFIG", 0).edit();
    ...
    private static void initProxyState() {
        try {
            packageName = MyApplication.getInstance().getPackageManager().getPackageInfo(MyApplication.getInstance().getPackageName(), 0).packageName;
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("=================包名获取失败，可能会影响代理请求功能=================");
        }
        proxyState = MyApplication.getInstance().getSharedPreferences("CONFIG", 0).getBoolean(SP_KEY_PROXY_STATE, PROXY_STATE_DEFAULT);
    }
    ...
    private static void initProxyHttp() {
        proxyHttp = MyApplication.getInstance().getSharedPreferences("CONFIG", 0).getString(SP_KEY_PROXY_HTTP, PROXY_HTTP_DEFAULT);
    }
